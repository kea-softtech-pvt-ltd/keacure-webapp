...
function EditMedicalData(props){
    ...
    const fetchPatientData =()=>{
        fetch(`http://localhost:9000/api/fetchUpdatedPatient/${medicalId}`).then(res =>{
            if(res){
                return res.json()
            }
        }).then(jsonRes => {
    ...
    const UpdatePatientData = async(event)=>{
        ...
        await axios.post(`http://localhost:9000/api/updatePatientMedicalInfo/${medicalId}` , updateMedical)
         .then((res)=>{
