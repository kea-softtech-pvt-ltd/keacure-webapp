...
function EditExperience(props){
    ...
    const getAllExperience =()=>{
        fetch(`http://localhost:9000/api/fetchUpdateExperience/${ExId}`).then(res =>{
            if(res){
                return res.json()
            }
        }).then(jsonRes => {
    ...
    async function UpdateData(e) {
        ...
        if(new Date(endYear).getTime() < new Date(startYear).getTime()) {
            ...
            const res = await axios.post(`http://localhost:9000/api/updateExperience/${ExId}` , updateExperienceData)
            .then((res)=>{
