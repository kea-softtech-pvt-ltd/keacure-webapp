...
function EditLifeStyleData(props){
    ...
    const fetchPatientData =()=>{
        fetch(`http://localhost:9000/api/fetchUpdatedPatientLifestyle/${lifeStyleId}`).then(res =>{
            if(res){
                return res.json()
            }
        }).then(jsonRes => {
    ...
    const UpdatePatientData = async(event)=>{
        ...
        await axios.post(`http://localhost:9000/api/updatePatientLifestyleInfo/${lifeStyleId}` , updateMedical)
         .then((res)=>{
